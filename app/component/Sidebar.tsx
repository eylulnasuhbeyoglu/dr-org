...
const Sidebar = ({ isOpen, setIsOpen, toggleSidebar }: SidebarProps) => {
  ...
  return (
    <>
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 shadow-lg z-30 transition-all duration-300 rounded-r-xl flex flex-col`}
          ...
          <button onClick={toggleSidebar} className="flex items-center gap-2">
            {isOpen ? (
              <>
                ...
                <span className="font-bold text-gray-800 dark:text-gray-100">Clinic</span>
              </>
            ) : (
              <Menu size={24} />
            )}
        ...
        <nav className="flex flex-col gap-2 p-2">
          {menuItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 px-3 py-2 rounded-md text-gray-700 dark:text-gray-200 hover:bg-blue-100 dark:hover:bg-gray-700 hover:text-blue-700 dark:hover:text-white transition"
            >
              {item.icon}
              <span
